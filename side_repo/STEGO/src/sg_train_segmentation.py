...
from side_repo.STEGO.src.data import ContrastiveSegDataset
from side_repo.STEGO.src.utils import *
from side_repo.STEGO.src.modules import *
from side_repo.STEGO.src.data import *
...
class LitUnsupervisedSegmenter(pl.LightningModule):
    def __init__(self, n_classes, cfg):
        ...
        if cfg.arch == "feature-pyramid":
            ...
            logger.warning("Using Feature Pyramid Network")
        elif cfg.arch == "dino":
            self.net = DinoFeaturizer(dim, cfg)
            logger.warning("Using DINO Featurizer")
        else:
        ...
        self.cluster_metrics = UnsupervisedMetrics(
            "test/cluster/", n_classes=n_classes,  extra_clusters=cfg.extra_clusters,  compute_hungarian=True)
        self.linear_metrics = UnsupervisedMetrics(
            "test/linear/", n_classes=n_classes, extra_clusters=0, compute_hungarian=False)
        ...
        if self.cfg.dataset_name.startswith("cityscapes"):
            ...
            logger.warning('using cityscapes colormap')
        else:
            self.label_cmap = create_pascal_label_colormap()
            logger.warning('using pascal colormap')
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.cfg.use_salience:  # default is False
        ...
        if self.cfg.rec_weight > 0:  # default 0.0
        ...
        if self.cfg.aug_alignment_weight > 0:  # default 0.0
        ...
        if self.cfg.crf_weight > 0:  # default 0.0
        ...
        if self.cfg.reset_probe_steps is not None and self.global_step == self.cfg.reset_probe_steps:
            logger.warning("wtf??? RESETTING PROBES")
        ...
        if self.global_step % 2000 == 0 and self.global_step > 0:
            logger.warning("RESETTING TFEVENT FILE")
    ...
    def validation_epoch_end(self, outputs) -> None:
        ...
        with torch.no_grad():
            ...
            if self.trainer.is_global_zero and not self.cfg.submitting_to_aml:
                ...
                fig, ax = plt.subplots(4, self.cfg.n_images, figsize=(
                    self.cfg.n_images * 3, 4 * 3))
                for i in range(self.cfg.n_images):
                    ...
                    ax[3, i].imshow(
                        self.label_cmap[self.cluster_metrics.map_clusters(output["cluster_preds"][i])])
                ...
                remove_axes(ax)
                plt.tight_layout()
                add_plot(self.logger.experiment,
                         "plot_labels", self.global_step)
                ...
                if self.cfg.has_labels:
                    ...
                    sns.heatmap(hist.t(), annot=False,
                    ...
                    colors = [self.label_cmap[i] /
                              255.0 for i in range(len(names))]
                    ...
                    ax.vlines(np.arange(0, len(names) + 1),
                              color=[.5, .5, .5], *ax.get_xlim())
                    ax.hlines(np.arange(0, len(names) + 1),
                              color=[.5, .5, .5], *ax.get_ylim())
                    plt.tight_layout()
                    add_plot(self.logger.experiment,
                             "conf_matrix", self.global_step)
                    ...
                    plt.tight_layout()
                    add_plot(self.logger.experiment,
                             "label frequency", self.global_step)
        ...
        if self.global_rank == 0:
            ...
            current_score = self.trainer.checkpoint_callback.current_score
...
def my_app(cfg: DictConfig) -> None:
    ...
    pytorch_data_dir = str(Path(pytorch_data_dir).expanduser().resolve())
    data_dir = join(cfg.output_root, "data")
    log_dir = join(cfg.output_root, "logs")
    checkpoint_dir = join(cfg.output_root, "checkpoints")

    prefix = "{}/{}_{}".format(cfg.log_dir,
    ...
    train_dataset = ContrastiveSegDataset(
        ...
        nice_dataset_name=cfg.nice_dataset_name,
    ...
    wandb.init(project='stego', entity="dpose-team", name=cfg.experiment_name,
    ...
    trainer = Trainer(
        ...
        callbacks=[
            ModelCheckpoint(
                ...
                mode="max",
                filename="epoch_{epoch:06}-testIoU_{test/cluster/mIoU:.2f}",# https://github.com/Lightning-AI/lightning/pull/12649/files
