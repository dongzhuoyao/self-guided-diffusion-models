...
class DirectoryDataset(Dataset):
    def __init__(self, root, path, image_set, transform, target_transform):
        ...
        logger.warning(self.img_dir, self.label_dir)
        ...
        if os.path.exists(join(self.dir, "labels")):
            ...
            logger.warning('Found {} images and {} labels'.format(
                len(self.img_files), len(self.label_files)))
        else:
            logger.warning(
                'No labels found, better only this directory is used for testing')
            self.label_files = None

    def __getitem__(self, index):
        image_filename = self.img_files[index]
        img = Image.open(join(self.img_dir, image_filename)).convert("RGB")
        ...
        if self.label_files is not None:
            ...
            label = torch.zeros(
                img.shape[1], img.shape[2], dtype=torch.int64) - 1
...
class Potsdam(Dataset):
    ...
    def __getitem__(self, index):
        ...
        img = to_pil_image(torch.from_numpy(img).permute(
            2, 0, 1)[:3])  # TODO add ir channel back
        try:
            label = loadmat(join(self.root, "gt", image_id + ".mat"))["gt"]
            label = to_pil_image(torch.from_numpy(
                label).unsqueeze(-1).permute(2, 0, 1))
...
class CroppedDataset(Dataset):
    def __init__(self, root, dataset_name, crop_type, crop_ratio, image_set, transform, target_transform):
        ...
        self.split = image_set
        self.root = join(root, "cropped", "{}_{}_crop_{}".format(
            dataset_name, crop_type, crop_ratio))
        self.transform = transform
        self.target_transform = target_transform
    ...
    def __getitem__(self, index):
        image = Image.open(
            join(self.img_dir, "{}.jpg".format(index))).convert('RGB')
...
class ContrastiveSegDataset(Dataset):
    def __init__(self,
        ...
        if dataset_name == "potsdam":
            ...
            extra_args = dict(coarse_labels=True)
        elif dataset_name == "directory":#main logic here
            ...
            extra_args = dict(dataset_name="cityscapes",
                              crop_type=crop_type, crop_ratio=cfg.crop_ratio)
        elif dataset_name == "cocostuff3":
            self.n_classes = 3
            dataset_class = Coco
            extra_args = dict(coarse_labels=True, subset=6,
                              exclude_things=True)
        elif dataset_name == "cocostuff15":
            self.n_classes = 15
            dataset_class = Coco
            extra_args = dict(coarse_labels=False,
                              subset=7, exclude_things=True)
        elif dataset_name == "cocostuff27" and crop_type is not None:
            self.n_classes = 27
            dataset_class = CroppedDataset
            extra_args = dict(dataset_name="cocostuff27",
                              crop_type=cfg.crop_type, crop_ratio=cfg.crop_ratio)
        elif dataset_name == "cocostuff27" and crop_type is None:
            self.n_classes = 27
            dataset_class = Coco
            extra_args = dict(coarse_labels=False,
                              subset=None, exclude_things=False)
            if image_set == "val":
                extra_args["subset"] = 7
        else:
        ...
        feature_cache_file = join(
            pytorch_data_dir, "nns", f"nns_{cfg.model_type}_{nice_dataset_name}_{image_set}_crop{crop_type}_res{cfg.res}.npz")

        if pos_labels or pos_images:
            if not os.path.exists(feature_cache_file) or compute_knns:
                raise ValueError("could not find nn file {} please run precompute_knns".format(
                    feature_cache_file))
            else:
                logger.warning(
                    "loading nns from {}".format(feature_cache_file))
                loaded = np.load(feature_cache_file)
                self.nns = loaded["nns"]
            assert len(self.dataset) == len(self.nns)
    ...
    def __getitem__(self, index):
        ...
        if self.pos_images or self.pos_labels:
            ind_pos = self.nns[index][torch.randint(
        ...
        ret = {
            "ind": index,
            "img": extra_trans(index, pack[0]),
            "label": extra_trans(index, pack[1]),
        }

        if self.pos_images:
            ret["img_pos"] = extra_trans(index, pack_pos[0])
            ret["ind_pos"] = ind_pos
        ...
        if self.pos_labels:
            ret["label_pos"] = extra_trans(index, pack_pos[1])
