...
@hydra.main(config_path="configs", config_name="config_base", version_base=None)
def my_app(cfg: DictConfig) -> None:
    ...
    if cfg.arch == "dino":
        ...
        cut_model = load_model(cfg.model_type, join(
    ...
    for crop_type in crop_types:
        for image_set in image_sets:
            feature_cache_file = join(
                pytorch_data_dir, "nns", f"nns_{cfg.model_type}_{cfg.nice_dataset_name}_{image_set}_crop{crop_type}_res{cfg.res}.npz")

            if not os.path.exists(feature_cache_file):
                logger.warning(
                    "{} not found, computing".format(feature_cache_file))
                ...
                loader = DataLoader(dataset, batch_size=cfg.nn_batch_size, shuffle=False,
                ...
                with torch.no_grad():
                    ...
                    logger.info("normed_feats.shape", normed_feats.shape)
                    ...
                    print("Saved NNs", cfg.model_type,
                          cfg.nice_dataset_name, image_set)
            else:
                logger.warning(
                    "{} found, skip re-computing".format(feature_cache_file))
