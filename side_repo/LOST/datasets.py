...
COCO_ROOT = '/home/thu/lab/self-guidance/side_repo/LOST/datasets/COCO/images/'

class ImageDIRDataset_V2():
    def __init__(self,img_dir):
        ...
        self.dataloader = DirDataset(img_dir=img_dir)


    def load_image(self, im_name):
        image = skimage.io.imread(f"{self.img_dir}/{im_name}.jpg")
        return image
...
class ImageDIRDataset:
    def __init__(self, image_path_root):
        
        self.image_path = image_path_root
        self.name = image_path_root.split("/")[-1]

        # Read the image
        with open(image_path_root, "rb") as f:
            img = Image.open(f)
            img = img.convert("RGB")

        # Build a dataloader
        img = transform(img)
        self.dataloader = [[img, image_path_root]]
...
class Dataset:
    def __init__(self, dataset_name, dataset_set, remove_hards):
        ...
        if dataset_name == "VOC07":
            ...
            #self.root_path = "/home/thu/dataset/VOC2012_Lost" #"datasets/VOC2012"
            self.root_path=str(Path('~/dataset/VOCdevkit/VOC2012').expanduser().resolve())
            self.year = "2012"
        elif dataset_name == "COCO20k":
            self.year = "2014"
            self.root_path = str(Path(f"~/lab/self-guidance/side_repo/LOST/datasets/COCO/images/{dataset_set}{self.year}").expanduser().resolve())
            self.sel20k = str(Path('~/lab/self-guidance/side_repo/LOST/datasets/coco_20k_filenames.txt').expanduser().resolve())
            # JSON file constructed based on COCO train2014 gt 
            self.all_annfile = str(Path("~/lab/self-guidance/side_repo/LOST/datasets/COCO/annotations/instances_train2014.json").expanduser().resolve())
            #self.annfile = "/home/thu/lab/self-guidance/side_repo/LOST/datasets/COCO/annotations/instances_train2014.json"
            self.annfile = str(Path("~/lab/self-guidance/side_repo/LOST/datasets/instances_train2014_sel20k.json").expanduser().resolve())
    ...
    def load_image(self, im_name):
        ...
        if "VOC" in self.dataset_name:
            image = skimage.io.imread(f"/home/thu/dataset/VOC2012_Lost/VOCdevkit/VOC{self.year}/JPEGImages/{im_name}")
        elif "COCO" in self.dataset_name:
            im_path = "train2014/" + "COCO_train2014_" + str(im_name).zfill(12) + '.jpg'
            image = skimage.io.imread(os.path.join(COCO_ROOT, im_path))
        else:
