...
class PerceiverResampler(nn.Module):
    def __init__(
        ...
        *,
        ...
        if num_latents_mean_pooled > 0:#mean_pool over all tokens and regenerate 'num_latents_mean_pooled' tokens to represent mean/global information... then concat with the original tokens 
...
class Unet(nn.Module):
    def __init__(
        ...
        *,
        ...
        use_global_context_attn = True,
         #############################
        cond_dim = None,  #will be set default by 'dim'
        num_resnet_blocks = 1,
        num_time_tokens = 2,
        ...
        # attention pooling by perceiver
    ...
    def forward_with_cond_scale_old(
        self,
        *args,
        cond_scale = 1.,
        **kwargs
    ...
    def forward(
        ...
        if exists(cond) and self.cond_on_text:
            if self.max_text_len==1:
                ...
                cond = rearrange(cond,'b c-> b 1 c')
            elif self.max_text_len>1:
            ...
            if exists(self.attn_pool):#if only one token, it will upsample to ~36, if too many token, it will downsample to ~36
            ...
            t = t + text_hiddens#check Section2.5 in Imagen paper, here is add not concat
...
if __name__ == '__main__':
    unet = BaseUnet64(max_text_len=1,text_embed_dim=2048)
    x = torch.randn(4,3,32,32)
    t = torch.randn(4,)
    #text_embeds = torch.randn(4,256, 2048)
    text_embeds = torch.randn(4, 2048)
    out = unet.forward(x,t,cond=text_embeds)
    print(out.shape)
