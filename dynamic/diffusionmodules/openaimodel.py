...
from diffusion_utils.util import count_params, slerp_batch_torch
...
from dynamic.vq_lr import VectorQuantize
...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        if condition_method == 'cluster_lookup':
            self.lookup_table = nn.Embedding(888888888, cond_dim)
            ...
            logger.info("has_cond for rebuttal!!!!!")
            logger.warning(
    ...
    def forward_with_cond_scale(
        self, x, t, cond_scale, cond,  layout=None, p0=None, image_batch_ids=None,
    ):
        if p0 is None:
        ...
        if is_number(cond_scale) and cond_scale == 1:
            epsilon_zc = self.forward(
                x=x, timesteps=t, cond_drop_prob=p0, cond=cond, layout=layout, image_batch_ids=image_batch_ids,
            )  # epsilon(z,c)
            ...
            epsilon_z = self.forward(
                x=x, timesteps=t, cond_drop_prob=p1, cond=cond, layout=layout, image_batch_ids=image_batch_ids,
            )  # epsilon(z)
            ...
            epsilon_cat = self.forward(
                ...
                image_batch_ids=concat_double_dim0(image_batch_ids),
            )  # epsilon(z,c)
    ...
    def forward(
        self, x, timesteps=None, cond=None, layout=None, cond_drop_prob=0.0, image_batch_ids=None,
    ):
        if self.condition_method == "cluster_lookup":
            # reset cond by look up table
            cond = self.lookup_table(image_batch_ids)
        if cond is not None:
            cond = cond.to(x.device).to(self.null_cond_emb.dtype)
        context = None
