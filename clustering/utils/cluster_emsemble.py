...
def clustering_ensemble(
    feat_h5_path, nns, pca_shuffle, cluster_k, niter, minp, cluster_h5_root=None, debug=False, ensemble_num=4,
):
    ...
    _cluster_h5_path = f"~/data/sg_data/cluster/v4_{dataset_name}_cluster{cluster_k}emsemble{ensemble_num}_iter{niter}minp{minp}_nns{nns}_{feat_from}_grey{is_grey}_{time_str}_{sha[:7]}.h5"
    if debug:
    ...
    f.create_dataset(
        "train", data=np.ones(shape=(len(get_feat("train")), ensemble_num), dtype=np.int64) * -1
    )
    f.create_dataset(
        "val", data=np.ones(shape=(len(get_feat("val")), ensemble_num), dtype=np.int64) * -1
    )
    ...
    for _,ens_id in enumerate(range(ensemble_num)):
        logger.warning(f"begin kmeans fit {ens_id}"+"#"*20)
        
        trainval_assigned, centroids = run_kmeans(
            feat_train=train_feat_original,
            feat_trainval=trainval_feat_original,
            cluster_k=cluster_k,
            niter=niter,
            minp=minp,
        )

        f["train"][:, ens_id] = trainval_assigned[:trainset_size,]
        f["val"][:, ens_id] = trainval_assigned[trainset_size:,]

        ##################
        if "train_labels" in f_feat and False:
